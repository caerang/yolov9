...
def run(
        ...
        save_imgs=False, # save result to images with GTs
        model=None,
        dataloader=None,
        save_dir=Path(''),
        plots=True,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        if plots and batch_i < 3 and save_imgs:
            print(len(im), len(paths))
            for path_idx, path in enumerate(paths):
                print(im[path_idx].shape, path)
...
def parse_opt():
    ...
    parser.add_argument('--save-imgs', action='store_true', help='save results to image with GTs')
